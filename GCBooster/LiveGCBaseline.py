...
def COP(T_amb_C):
    ...
    h_gc_out = CP.PropsSI('H', 'P', P_gc, 'T', T_gc_out, 'CO2')
    h_in_rec = h_gc_out
    ...
    eta_MT = -0.0144 * ((P_gc / P_evap_MT) ** 2) + 0.0719 * (P_gc / P_evap_MT) + 0.5826
    s_discharge_MT_is = s_suction_MT
    h_discharge_MT_is = CP.PropsSI('H', 'P', P_gc, 'S', s_discharge_MT_is, 'CO2')
    ...
    Q_dot_GC = (m_dot_MT + m_dot_LT + m_dot_vbv) * (h_discharge_MT - h_gc_out) / 0.97 / 1000
    W_dot_fan = Q_dot_GC * 0.03

    COP = (Q_dot_LT + Q_dot_MT) / (W_dot_MT + W_dot_LT + W_dot_fan)

    # print(f"COP: {COP}")
    """If you want static plot of states"""
    ...
    # plot_point(h_gc_out, P_gc, 'GC_out')
    ...
    # plot_point(h_discharge_MT, P_gc, 'MT_discharge')
    # plt.grid(True)
    """___________________________________________________________________"""

    """If you want a dynamic plot"""
    S1  = [h_out_rec_liq, P_rec, 'Rec_liq']
    S2  = [h_out_rec_vap, P_rec, 'Rec_vap']
    S3  = [h_in_evap_MT, P_evap_MT, 'MT_evap_in']
    S4  = [h_out_evap_MT, P_evap_MT, 'MT_evap_out']
    S5  = [h_in_evap_LT, P_evap_LT, 'LT_evap_in']
    S6  = [h_out_evap_LT, P_evap_LT, 'LT_evap_out']
    S7  = [h_suction_LT, P_evap_LT, 'LT_suction']
    S8  = [h_discharge_LT, P_evap_MT, 'LT_discharge']
    S9  = [h_gc_out, P_gc, 'GC_out']
    S10 = [h_in_rec, P_rec, 'Rec_in']
    S11 = [h_out_vbv, P_evap_MT, 'VBV_out']
    S12 = [h_mix_1, P_evap_MT, 'Mix1']
    S13 = [h_mix_2, P_evap_MT, 'Mix2']
    S14 = [h_suction_MT, P_evap_MT, 'MT_suction']
    s15 = [h_discharge_MT, P_gc, 'MT_discharge']
    # print(COP,[S1 , S2 , S3 , S4 , S5 , S6 , S7 , S8 , S9 , S10, S11, S12, S13, S14, s15])
    return COP,[S1 , S2 , S3 , S4 , S5 , S6 , S7 , S8 , S9 , S10, S11, S12, S13, S14, s15]
